        ...
        for epoch in range(epochs):
            ...
            if test_images and epoch % test_frequency == 0:
                for img_path in test_images:
                    ...
                    # Load an generate images
                    img_hr, img_lr = loader.load_batch(batch_size=1, img_paths=[img_path])
                    ...
                    for i, (title, img) in enumerate(images.items()):
                        axes[i].imshow(np.squeeze(0.5 * img + 0.5, axis=0))
                        axes[i].set_title(title)
                        axes[i].axis('off')
                    
                    # Save and close the figure
                    fig.savefig(f"{test_output}sample-epoch{epoch}.png")
                    plt.close()
